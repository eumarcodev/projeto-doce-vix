...
export interface IProductPrisma extends Omit<PProduct, 'category' | 'dayOfWeek' | 'file'> {
    category: ICategory;
    ...
    implements IDefaultFactory<IProductPrisma, IProduct>
{
    async generate(entity: IProductPrisma): Promise<IProduct> {
        const {
            id, guid, name, description, price, createdAt, updatedAt, category, dayOfWeek, files
        } = entity;
        return {
            id,
            guid,
